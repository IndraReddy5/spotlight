    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <main class="me-lg-auto ms-md-auto px-md-4 pt-5 main_body">
                <div>
                    <h1 style="color:var(--maize);" class="text-center" v-if="AlbumName != '' && AlbumName != 'Album not found'"> {{ AlbumName }}
                    </h1>
                    <h1 v-if="AlbumName == 'Album not found'" class="text-center"> Album Not Found</h1>
                    <h6 class="text-center" v-if="SearchedSongs.length > 1"> Songs are sorted by Rating</h6>
                    <br>
                    <div class="p-3 bg-dark rounded box-shadow" v-if="getSongsLength() >= 1">
                        <div class="media pt-3 d-flex justify-content-between mb-0 pb-3 border-bottom"
                            v-for="value, key, index in SearchedSongs">
                            <div class="d-flex justify-content-start">
                                ...
                                <p class="media-body small lh-125 ms-2 pt-1">
                                    <strong class="d-block pt-1"><a :href="generateSongURL(value['song_id'])">{{
                                        value['song_name'] }}</a></strong>
                                    <strong class="d-block pt-1 text-secondary">Album: {{ value['album_name']
                                    }}</strong>
                    ...
                    <p class="text-center" v-else> <span v-if="AlbumName != '' && AlbumName != 'Album not found'"> You don't have any songs in this album yet. </span> </p>
                </div>
            </main>
        </div>
    </div>
...
export default {
    ...
    async beforeMount() {
        ...
        await fetch(__API_URL__ + "songs?album_id=" + this.AlbumID, { headers: headers, method: "GET" })
            .then(response => {
                if (response.ok) {
            ...
            .then(data => {
                ...
                delete this.SearchedSongs['album_name']
            })
            .catch((error) => {
                this.errormsg = error;
                console.log(error)
